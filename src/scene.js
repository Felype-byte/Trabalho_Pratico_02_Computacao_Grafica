...
// ==========================================
// 6. ENQUADRAMENTO DA CÂMERA (CORRIGIDO PARA WIDESCREEN)
// ==========================================
export function updateCamera(camera, player, screenWidth) {
    ...
    // Movimento suave a seguir o jogador
    ...
    // Limite máximo do cenário (as paredes estão em -700 e 700)
    const limiteCenario = 700;
    
    // Calcula o limite dinâmico subtraindo a largura da visão atual da câmara
    let minCamX = -limiteCenario + camera.right;
    let maxCamX = limiteCenario - camera.right;

    // Se o monitor for tão largo que é maior que a própria fase inteira
    if (minCamX > maxCamX) {
        targetX = 0; // Tranca a câmara perfeitamente no centro
    } else {
        // Aplica o limite dinâmico para não mostrar as bordas pretas
