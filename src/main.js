...
// ==========================================
// 1. CARREGAMENTO DE TEXTURAS E EFEITOS
// ==========================================
...
function calculateFinalRank(timeInSeconds, damageTaken, enemiesDefeated) {
    const basePoints = 400;
    const killPoints = enemiesDefeated * 25;
    const timeBonus = Math.max(0, 300 - (timeInSeconds * 1.5));
    const survivalBonus = Math.max(0, 300 - (damageTaken * 30));

    const totalScore = basePoints + killPoints + timeBonus + survivalBonus;

    let rank = "C";
    if (totalScore >= 1100) rank = "S+";
    else if (totalScore >= 950) rank = "S";
    else if (totalScore >= 750) rank = "A";
    else if (totalScore >= 550) rank = "B";

    return { rank, totalScore, killPoints, timeBonus, survivalBonus };
}
...
function endPhaseWithVictory() {
    ...
    const result = calculateFinalRank(totalTime, totalDamageTaken, totalEnemiesDefeated);

    uiWin.classList.add('active');
    uiWin.innerHTML = `
        <div id="win-panel" class="ui-panel center-panel">
            <h1 class="ui-title">ARENA CONCLUÍDA!</h1>
            <div class="rank-display">${result.rank}</div>
            <p class="ui-text">Pontos Totais: <span class="ui-highlight">${Math.floor(result.totalScore)}</span></p>
            
            <div style="margin: 15px auto; width: 85%; text-align: left; font-size: 0.9rem; opacity: 0.85; line-height: 1.6;">
                <div>⏱️ Tempo: ${totalTime}s (Bônus: +${Math.floor(result.timeBonus)})</div>
                <div>💥 Dano Sofrido: ${totalDamageTaken} HP (Bônus: +${Math.floor(result.survivalBonus)})</div>
                <div>⚔️ Abates: ${totalEnemiesDefeated} Inimigos (+${Math.floor(result.killPoints)})</div>
            </div>
            <br>
...
// ==========================================
// 12. REDIMENSIONAMENTO DINÂMICO DE TELA
// ==========================================
window.addEventListener('resize', () => {
    const width = window.innerWidth;
    const height = window.innerHeight;
    
    // Atualiza o tamanho do renderizador para preencher toda a janela
    renderer.setSize(width, height);
    
    // Mantém a altura do jogo fixa, mas expande a largura (Widescreen adaptável)
    const aspect = width / height;
    const viewHeight = HEIGHT; // Mantém os 600 originais para não quebrar a física
...
// Força o ajuste logo na primeira vez que o jogo abre
